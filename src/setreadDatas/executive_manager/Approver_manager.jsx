...
const Approver_manager = ({}) => {
  ...
  const getEmployeeDB = async () => {
    try {
      const { data } = await apiClient.get(
        `/show_Recruitment_approval_manager?PerWP=${_PerWP}&PerD=${PerDs}`
      );
  ...
  const handleApprovalChange = async (index, value, orderNumber) => {
    ...
    try {
      ...
      console.log(payload);
      const response = await apiClient.post("/DataApprovals_manager", payload);

      // let data = response;

      // const { status, result, message } = response.data;
      // if (status) {
      //   console.log("ข้อมูลที่ backend ส่งมา:", message);
      // } else {
      //   console.error("Error: Status is not true. Received data:", data);
      // }
          ...
          {getemployee.length > 0 ? (
            ...
            >
              ...
              >
                ...
                <div className="col-md-2">
                  <center>จำนวนวันที่เหลือ / วันที่มีผล</center>
                </div>
                <div className="col-md-2">
                      ...
                      {employee.effective_date ? (
                        <>
                          {Math.max(
                            0,
                            Math.ceil(
                              (new Date(employee.effective_date) - new Date()) /
                                (1000 * 60 * 60 * 24)
                            )
                          )}{" "}
                          วัน
                  ...
                  >
                    ...
                    >
                      <input
                        type="radio"
                        name={`approve-${index}`}
                        checked={approvals[index]?.[0] === true} // ✅ จะเช็คค่า value เท่านั้น
                        onChange={() =>
                    ...
                    >
                      <input
                        type="radio"
                        name={`approve-${index}`}
                        checked={approvals[index]?.[0] === false} // ดูเฉพาะ value
                        onChange={
                          () =>
                            handleApprovalChange(
                              index,
                    ...
                    <button
                      ...
                      onClick={() => handleApprovalChange(index, null)}
                    >
                      ล้าง
                    </button> */}
                  </div>
          ...
          <Typography id="document-modal-title" variant="h6" component="h2">
            ...
            <p>
              ...
              <div className="text-end pt-3">
                ...
                <p style={{ fontSize: "14px" }}>
                  <>
                    {docData[0]?.hr_fullname} ยื่นแบบฟอร์มเมื่อ{" "}
...
export default Approver_manager;
