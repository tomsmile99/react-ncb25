...
const Approver_contain = ({
  ...
  const getEmployeeDB = async () => {
    try {
      const { data } = await apiClient.get(
        `/show_Recruitment_approval_gm?PerWP=${_PerWP}&PerD=${PerDs}`
      );
  ...
  const handleApprovalChange = async (index, value, orderNumber) => {
    ...
    try {
      ...
      console.log(payload)
      const response = await apiClient.post("/DataApprovals_gm", payload);
...
export default Approver_contain;
